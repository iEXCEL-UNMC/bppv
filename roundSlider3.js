	...
	*/
	initRange(range) {
		...
		if (circular) {
			...
			/*
			document.getElementById("rotX_leftButton").addEventListener('mousedown', (event) =>{

				event.preventDefault(); this.range.stepUp(); this.updateCircle(); 
			})
			*/
			const pointerMove = (event) => { 
				
				console.log(this.wrapper.dataset.value);
